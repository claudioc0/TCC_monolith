...
class OrderServiceTest {
    ...
    @Test
    @DisplayName("confirm delega ao método confirm da entidade")
    void confirmDelegatesToEntity() {
        Order order = new Order(1L, List.of(new OrderItem(1L, 1, new BigDecimal("50.00"))));
        when(orderRepository.findById(1L)).thenReturn(Optional.of(order));
        when(orderRepository.save(order)).thenReturn(order);
    ...
    @Test
    @DisplayName("ship delega ao método ship da entidade")
    void shipDelegatesToEntity() {
        Order order = new Order(1L, List.of(new OrderItem(1L, 1, new BigDecimal("50.00"))));
        order.confirm();
        when(orderRepository.findById(1L)).thenReturn(Optional.of(order));
        when(orderRepository.save(order)).thenReturn(order);
    ...
    @Test
    @DisplayName("deliver delega ao método deliver da entidade")
    void deliverDelegatesToEntity() {
        Order order = new Order(1L, List.of(new OrderItem(1L, 1, new BigDecimal("50.00"))));
        order.confirm();
        order.ship();
        when(orderRepository.findById(1L)).thenReturn(Optional.of(order));
