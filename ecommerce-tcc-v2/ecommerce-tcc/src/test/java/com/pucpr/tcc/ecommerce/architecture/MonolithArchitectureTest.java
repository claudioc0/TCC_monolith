...
class MonolithArchitectureTest {
    ...
    @Test
    @DisplayName("[user] Domínio não deve depender de infraestrutura")
    void userDomainShouldNotDependOnInfrastructure() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..user.domain..")
                .should().dependOnClassesThat().resideInAPackage("..user.infrastructure..");
        rule.check(importedClasses);
    }

    @Test
    @DisplayName("[user] Domínio não deve depender de Spring")
    void userDomainShouldNotDependOnSpring() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..user.domain..")
                .should().dependOnClassesThat().resideInAPackage("org.springframework..");
        rule.check(importedClasses);
    ...
    @Test
    @DisplayName("[user] Application não deve depender de infraestrutura")
    void userApplicationShouldNotDependOnInfrastructure() {
        ArchRule rule = noClasses()
    ...
    @Test
    @DisplayName("[report] Não deve depender da infraestrutura de order")
    void reportShouldNotDependOnOrderInfrastructure() {
        ArchRule rule = noClasses()
    ...
    @Test
    @DisplayName("[report] Service não deve depender de infraestrutura própria")
    void reportServiceShouldNotDependOnOwnInfrastructure() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..report.service..")
                .should().dependOnClassesThat().resideInAPackage("..report.infrastructure..");
        rule.check(importedClasses);
    }
