...
public class OrderController {
    ...
    public ResponseEntity<OrderResponse> confirm(@PathVariable Long id) {
        return ResponseEntity.ok(OrderResponse.from(orderService.confirm(id)));
    }

    @PatchMapping("/{id}/ship")
    public ResponseEntity<OrderResponse> ship(@PathVariable Long id) {
        return ResponseEntity.ok(OrderResponse.from(orderService.ship(id)));
    }

    @PatchMapping("/{id}/deliver")
    public ResponseEntity<OrderResponse> deliver(@PathVariable Long id) {
        return ResponseEntity.ok(OrderResponse.from(orderService.deliver(id)));
    }
}
